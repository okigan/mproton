...
class CWideCharToMultiByteConverter {
   public:
    CWideCharToMultiByteConverter(LPCWSTR psz) {
        ...
        if (psz != NULL) {
            ...
            // TODO: optimize/cache this allocation
            int lenMB =
                WideCharToMultiByte(nCodePage, 0, psz, lenWC, NULL, 0, NULL, NULL);
            m_psz = new CHAR[lenMB];
            int result =
                WideCharToMultiByte(nCodePage, 0, psz, lenWC, m_psz, lenMB, NULL, NULL);
            if (result == 0) {
                DWORD dwError = ::GetLastError();
                throw dwError;
            }
   ...
   private:
    ...
    CWideCharToMultiByteConverter& operator=(
        const CWideCharToMultiByteConverter&) throw();
};
...
class Handler : public ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler,
   ...
   public:
    ...
    HRESULT Invoke(ICoreWebView2* sender,
    ...
    HRESULT Invoke(HRESULT errorCode, LPCWSTR resultObjectAsJson) {
        std::cout << "in ICoreWebView2ExecuteScriptCompletedHandler Invoke:" << errorCode
                  << CW2A(resultObjectAsJson) << std::endl;
        return S_OK;
    }
};
...
int prtn_initialize(void) {
    ...
    HANDLE himage = LoadImage(hInstance,
                              IDI_APPLICATION,
                              IMAGE_ICON,
